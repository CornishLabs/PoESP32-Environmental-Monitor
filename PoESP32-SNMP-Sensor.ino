...
IPAddress ip(192, 168, 23, 99);     // Set device IP address.
IPAddress gateway(192, 168, 23, 1); // Set default gateway IP address.
...
const IPAddress AUTHORIZED_HOSTS[2] = {IPAddress(192, 168, 23, 1), IPAddress(192, 168, 23, 110)};
          ...
          if (memcmp(SNMP_READCOMMUNITY_LEN_6, payload + 6, sizeof(SNMP_READCOMMUNITY_LEN_6)) == 0)
          {
            ...
            if (memcmp(SNMP_READCOMMUNITY_VALUE_7, payload + 7, sizeof(SNMP_READCOMMUNITY_VALUE_7) - 1) == 0)
            {
              ...
              if (memcmp(SNMP_GETREQUEST_7_LEN, payload + 6 + sizeof(SNMP_READCOMMUNITY_VALUE_7), sizeof(SNMP_GETREQUEST_7_LEN)) == 0)
              {
                ...
                if (9 + sizeof(SNMP_READCOMMUNITY_VALUE_7) - 1 + RIDLength + 2 + 6 + 6 > length)
                  return -1;                                                                                                // Prevent out-of-bounds memory read
